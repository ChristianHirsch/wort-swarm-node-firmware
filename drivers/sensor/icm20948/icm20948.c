...
int icm20948_set_interrupt(struct device *dev, uint8_t value)
{
	...
	if (data->hw_tf->write_reg(data, ICM20948_REG_INT_ENABLE, value)) {
		LOG_DBG("failed to set interrupt enable register");
	}
...
int icm20948_init(struct device *dev)
{
	...
	data->bank = 0;
	if (!data->bus) {
		LOG_DBG("master not found: %s", DT_INST_BUS_LABEL(0));
		return -EINVAL;
	}
...
DEVICE_AND_API_INIT(icm20948, DT_INST_LABEL(0), icm20948_init,
